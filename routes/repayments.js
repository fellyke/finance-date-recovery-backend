...
function normalizeLoanNumber(value) {
    ...
    let loanNumber = String(value).trim();

    if (/^\d+\.0$/.test(loanNumber)) {
        loanNumber = loanNumber.replace(".0", "");
    }
...
function getBodyValue(body, snakeCase, camelCase) {
    if (body[snakeCase] !== undefined) {
...
router.get("/", authenticateToken, async (req, res) => {
    ...
    try {
        ...
        // Loan number
        if (loanNumber && String(loanNumber).trim() !== "") {

            const normalizedLoan =
                normalizeLoanNumber(loanNumber);

            values.push(`%${normalizedLoan}%`);
        ...
        if (
            ...
            conditions.push(
                `receipt_number ILIKE $${values.length}`
            );
        }
